...
describe("HealthCheckService", () => {
  ...
  describe("handleAlert", () => {
    ...
    it("should send alert when threshold is reached", async () => {
      ...
      const sendAlertSpy = vi.spyOn(alertService, "sendAlert").mockResolvedValue();
      ...
      expect(sendAlertSpy).toHaveBeenCalledWith(site, domain, failureData, 3);
    });
  ...
  describe("checkSite", () => {
    it("should use the default timeout when the site omits one", async () => {
      const fetchSpy = vi
        .spyOn(global, "fetch")
        .mockResolvedValue(/** @type {Response} */ ({ ok: true, status: 200 }));
      const setTimeoutSpy = vi.spyOn(global, "setTimeout");

      await service.checkSite({ url: "https://example.com" });
    ...
    it("should clear failure count after successful checks", async () => {
      vi.spyOn(global, "fetch").mockResolvedValue(
        /** @type {Response} */ ({ ok: true, status: 200 }),
      );
      await storage.put("failure-count:example.com", "2");

      await service.checkSite({ url: "https://example.com", timeout: 10000 });

      expect(await storage.get("failure-count:example.com")).toBeNull();
    });

    it("should store current status for successful checks", async () => {
      vi.spyOn(global, "fetch").mockResolvedValue(
        /** @type {Response} */ ({ ok: true, status: 204 }),
      );

      await service.checkSite({ url: "https://example.com", expectedStatus: 204 });

      const status = await service.getCurrentStatus("example.com");
    ...
    it("should support expected response text and custom methods", async () => {
      ...
      const status = await service.checkSite({
    ...
    it("should retry failing checks before storing a failure", async () => {
      vi.spyOn(global, "fetch")
        .mockRejectedValueOnce(new Error("Network error"))
        .mockResolvedValueOnce(/** @type {Response} */ ({ ok: true, status: 200 }));
      ...
      const today = new Date().toISOString().split("T")[0];
      const failures = await storage.list({ prefix: `failure:example.com:${today}:` });
      expect(failures.keys).toHaveLength(0);
    });

    it("should send recovery notifications after an alerted outage recovers", async () => {
      vi.spyOn(global, "fetch").mockResolvedValue(
        /** @type {Response} */ ({ ok: true, status: 200 }),
      );
      const recoverySpy = vi.spyOn(alertService, "sendRecovery").mockResolvedValue();
      await storage.put("alert-state:example.com", "down");
      await storage.put("failure-count:example.com", "3");

      const status = await service.checkSite({ url: "https://example.com" });
      ...
      expect(await storage.get("alert-state:example.com")).toBeNull();
    });

    it("should suppress checks when a site is paused", async () => {
      const fetchSpy = vi.spyOn(global, "fetch");

      const status = await service.checkSite({ url: "https://example.com", paused: true });
  ...
  describe("updateDailySummaries", () => {
    ...
    it("should use service session start when it is more recent than start of day", async () => {
      ...
      const thirtyMinutesAgo = Date.now() - 30 * 60 * 1000;
      const serviceWithRecentStart = new HealthCheckService(
        storage,
        config,
        alertService,
        thirtyMinutesAgo,
