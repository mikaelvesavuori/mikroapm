...
describe("AlertService", () => {
  ...
  describe("sendAlert", () => {
    ...
    it("should send down webhook notifications", async () => {
      config = ConfigManager.create(
        storage,
        { WEBHOOK_URL: "https://hooks.example.com/apm", WEBHOOK_SECRET: "secret" },
        { sites: [{ url: "https://example.com" }] },
      );
      service = new AlertService(config);
      fetchSpy.mockResolvedValue({ ok: true });
      ...
      await service.sendAlert(
        ...
        "example.com",
        { status: 500, message: "HTTP 500", duration: 100 },
        3,
      );

      expect(fetchSpy).toHaveBeenCalledWith(
        "https://hooks.example.com/apm",
        expect.objectContaining({
          method: "POST",
          headers: expect.objectContaining({
            Authorization: "Bearer secret",
            "x-mikroapm-webhook-secret": "secret",
          }),
        }),
      );

      const body = JSON.parse(fetchSpy.mock.calls[0][1].body);
      expect(body.event).toBe("site.down");
      expect(body.domain).toBe("example.com");
      expect(body.data.consecutiveFailures).toBe(3);
    });

    it("should send recovery webhook notifications", async () => {
      config = ConfigManager.create(
        storage,
      ...
      await service.sendRecovery({ url: "https://example.com" }, "example.com", {
        status: "operational",
      });

      const body = JSON.parse(fetchSpy.mock.calls[0][1].body);
      expect(body.event).toBe("site.recovered");
